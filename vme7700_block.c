...
main(int argc, char **argv)
{
 ...
 return 0;
usage:
  fprintf(stderr, "Usage: %s read <slot number> <addr> <datasize(words)> \n", argv[0]);
  fprintf(stderr, "       %s write <slot number> <addr> <value>\n", argv[0]);
  fprintf(stderr, "where datasize is decimal, addr and value are hex\n");
  return -1;
}
